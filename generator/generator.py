...
class Generator(nn.Module):
    def __init__(self, vocabs,word_char_dim, word_dim, concept_char_dim,
        ...
        self.relation_encoder = RelationEncoder2(vocabs['relation'], rel_dim,
                rnn_hidden_size, rnn_num_layers, dropout)
        self.bag_encoder = BagEncoder(vocabs['relation'].padding_idx, rel_dim,
                char2word_dim, embed_dim, rnn_hidden_size, dropout, num_heads)
    ...
    def con_rel_enc(self, inp):
        ...
        relation = self.relation_encoder(inp['relation_bank'],
                inp['relation_length'])
        relation = self.bag_encoder(relation, inp['relation'], inp['td_bags'],
